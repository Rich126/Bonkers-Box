...
- Speedway live timing, final replay timing, completed laps, total times and saved fastest-lap records now display to three decimal places.
- Existing fastest-lap records are retained unchanged as raw millisecond values.
- The storage key remains `spencerSpeedwayFastestLapsV1`; no migration or reset is performed.
- Speedway physics, handling, record comparisons, rider-name matching and leaderboard ranking are unchanged.
- Updated visible build markers and relevant cache keys to v4.9.
