(function(){
  ...
  function impressionHistory(state=gs()){return Array.isArray(state&&state.impressionTargetHistory)?state.impressionTargetHistory.map(String):[];}
  function playerImpressionState(round,state=gs()){
    const players=currentPlayers.filter(p=>p&&p.id&&cleanName(p.name));if(!round||round.impressionTarget!=='player'||!players.length)return {impressionTargetHistory:impressionHistory(state)};
    let history=impressionHistory(state).filter(id=>players.some(p=>String(p.id)===id));let pool=players.filter(p=>!history.includes(String(p.id)));if(!pool.length){history=[];pool=players.slice();}
    const target=pool[Math.floor(Math.random()*pool.length)],templates=Array.isArray(round.promptTemplates)&&round.promptTemplates.length?round.promptTemplates:[round.prompt||'Do your best impression of {player}!'];const template=templates[Math.floor(Math.random()*templates.length)];
    return {promptOverride:String(template).replace(/\{player\}/g,target.name),impressionTargetPlayerId:target.id,impressionTargetName:target.name,impressionTargetHistory:[...history,String(target.id)]};
  }
  function creativePrompt(round,state=gs()){return String((state&&state.promptOverride)||(round&&round.prompt)||'Get ready…');}
  function creativeKindLabel(round){if(round&&round.creativeType==='impression')return '🎭 IMPRESSION';return round&&round.kind==='picture'?'📸 PICTURE':'🎙️ SOUND';}
  function newQuestionState(index,extra={}){return {phase:'question',questionIndex:index,startedAt:new Date().toISOString(),paused:false,pausedStartedAt:null,pauseMs:0,impressionTargetHistory:impressionHistory(),...extra};}
  function newCreativeCaptureState(index,extra={}){const r=extra.creativeId?mixCreativeById.get(String(extra.creativeId)):creativeAt(index),dynamic=playerImpressionState(r);return {phase:'creative_capture',creativeIndex:index,kind:r?r.kind:'picture',startedAt:new Date().toISOString(),voteStage:null,heatIndex:0,heatGroups:[],finalists:[],candidateIds:[],stageKey:null,lastWinners:[],stageVoteCounts:{},scored:false,...dynamic,...extra};}
  ...
  function renderCreativeCapture(){
    ...
    const mixed=settingsOf().gameKey==='mix',current=mixed?Number(state.mixIndex):Number(state.creativeIndex),total=mixed?mixTotal():creative.rounds.length;$('creative-kind').textContent=creativeKindLabel(round)+(mixed?' • MIX':'');$('creative-round-title').textContent=round.title+' • Round '+(current+1)+' of '+total;$('creative-progress').style.width=((current+1)/total*100)+'%';$('creative-prompt').textContent=creativePrompt(round,state);
  ...
  async function startSoundRecording(){
    ...
    try{
      ...
      const round=creativeRoundForState(),maxRecordSeconds=clamp(Number(round&&round.recordSeconds)||10,5,30);mediaRecorder.start();$('record-sound').classList.add('recording');$('record-sound').textContent='●';$('stop-sound').classList.remove('hidden');$('sound-status').textContent='Recording… maximum '+maxRecordSeconds+' seconds';recordingStopTimer=setTimeout(stopSoundRecording,maxRecordSeconds*1000);
  ...
  async function renderCreativeVote(){
    const token=++renderSerial,state=gs(),round=creativeRoundForState(state);if(!round)return;syncMusic('creative');stopTimers();stopAllMedia();show('creative-vote');$('vote-stage-pill').textContent=state.voteStage==='heat'?'🔥 HEAT '+(Number(state.heatIndex)+1)+'/'+(state.heatGroups||[]).length:'🏆 GRAND FINAL';$('vote-title').textContent=round.creativeType==='impression'?(state.voteStage==='heat'?'Vote for the best impression':'Vote for the best impression'):(state.voteStage==='heat'?'Vote for the heat winner':'Vote for the overall winner');$('vote-prompt').textContent=creativePrompt(round,state);renderCreativeCounts();
  ...
  async function renderCreativeResult(){
    const token=++renderSerial,state=gs(),round=creativeRoundForState(state);if(!round)return;syncMusic('result');stopTimers();stopAllMedia();show('creative-result');$('result-prompt').textContent=creativePrompt(round,state);const heat=state.voteStage==='heat',winners=new Set(state.lastWinners||[]),counts=state.stageVoteCounts||{};$('creative-result-heading').innerHTML=heat?'🔥 <span>Heat Result</span>':'🏆 <span>Round Result</span>';
